#> nnsb_mobs:mob_name/tester/blue
#ステータスの設定
##モブの召喚
  summon stray ~ ~0.5 ~ {Tags:["status_set","enemy"]}

#名前を設定する
  data modify storage nnsb: status.name set value '{"text":"test","color":"#0000ff"}'
#HPを設定する
...
#ノクバ耐性
  data modify storage nnsb: status.knock_back.resistance set value 1d
#属性設定
  data modify storage nnsb: status.tag set value "blue"
#サイレント
  data modify storage nnsb: status.silent set value true

##装備
#ヘルメット
  data modify storage nnsb: status.armor.set_head set value {id:"minecraft:purple_bed",Count:1b,tag:{CustommodelData:0}}
#チェスト
  data modify storage nnsb: status.armor.set_body set value {id:"minecraft:名前",Count:1b,tag:{CustommodelData:0}}
#レギンス
...
#メインハンド
  data modify storage nnsb: status.hand.main set value {id:"minecraft:名前",Count:1b,tag:{CustommodelData:0}}
#オフハンド
  data modify storage nnsb: status.hand.sub set value {id:"minecraft:名前",Count:1b,tag:{CustommodelData:0}}
