#> nnsb_item:crafts_item/custom_craft_table/tick
...
#クラフトしたら
  ...
  execute as @e[type=item_frame,nbt={ItemRotation:1b}] run stopsound @a ambient entity.item_frame.rotate_item
  execute as @e[type=item_frame,nbt={ItemRotation:1b}] run playsound minecraft:item.crossbow.shoot ambient @a ~ ~ ~ 2 2
