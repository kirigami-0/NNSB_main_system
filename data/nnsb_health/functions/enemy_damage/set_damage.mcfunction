...
#取得した敵のスコアボードを減算
  execute at @e[tag=math_hp] as @e[limit=1,sort=nearest] run scoreboard players operation @s mob_hp -= &damage_set define 

#HPが0以下になったら以下の処理を行う
  #死んだときと同じルートテーブルを呼び出す
    execute at @e[tag=math_hp] as @e[limit=1,sort=nearest,scores={mob_hp=..0}] run loot spawn ~ ~ ~ kill @s
  ...
  #殺すとバグるので奈落にTP
    execute at @e[tag=math_hp] as @e[limit=1,sort=nearest,scores={mob_hp=..0}] run tp @s ~ ~-3000 ~

#マーカーを削除
  execute at @e[tag=math_hp] run kill @e[limit=1,sort=nearest,type=armor_stand]